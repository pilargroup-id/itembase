...
async function store(req, res, next) {
  try {
    const result = await PortService.store(req.body, req.user.id, req);
...
async function update(req, res, next) {
  try {
    const result = await PortService.update(req.params.id, req.body, req.user.id, req);
...
async function destroy(req, res, next) {
  try {
    const result = await PortService.destroy(req.params.id, req.user.id, req);
...
async function toggleStatus(req, res, next) {
  try {
    const result = await PortService.updateStatus(
      req.params.id,
      req.body.is_active,
      req.user.id,
      req
