...
async function exportData(req, res, next) {
  try {
    return sendFile(res, await ExportService.exportItems(req.query));
  } catch (err) {
    return next(err);
  }
}
...
async function inactiveItems(req, res, next) {
  try {
    ...
    return response.paginated(
      ...
      result.data,
      result.meta,
      'Inactive items retrieved successfully'
    );
  } catch (err) {
    return next(err);
  }
...
async function template(req, res, next) {
  try {
    return sendFile(res, await TemplateService.generate(req.params.type));
  } catch (err) {
    return next(err);
  }
}
...
async function commit(req, res, next) {
  try {
    const result = await ImportService.commit(req.body.preview_token, req.user.id);
    return response.ok(res, result, 'Import committed');
  } catch (err) {
    return next(err);
  }
}
...
async function cancel(req, res, next) {
  try {
    await ImportService.cancel(req.params.token, req.user.id);
    return response.ok(res, null, 'Import preview canceled');
  } catch (err) {
    return next(err);
  }
}
...
async function errorFile(req, res, next) {
  try {
    return sendFile(res, await ImportService.errorFile(req.params.token, req.user.id));
  } catch (err) {
    return next(err);
  }
}
