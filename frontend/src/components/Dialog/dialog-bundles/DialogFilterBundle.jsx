...
function DialogFilterBundle({
  isOpen = false,
  eyebrow = "Filter",
  title = "Bundle Table Filter",
  filterConfigs = [],
  filterFieldOptions = [],
  selectedFilterKeys = [],
  ...
  const dialogNode = (
    <div className="dashboard-popup-overlay" role="presentation" onClick={onClose}>
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="dialog-filter-bundle-title"
        onClick={(event) => event.stopPropagation()}
      >
        <div className="dashboard-popup__header">
          <div>
            <p className="dashboard-popup__eyebrow">{eyebrow}</p>
            <h2 className="dashboard-popup__title" id="dialog-filter-bundle-title">
              {title}
            </h2>
          </div>
        ...
        <div className="dashboard-popup__body parent-filter-popup__body">
          <div className="parent-filter-popup__layout">
            <section className="parent-filter-popup__section" aria-labelledby="bundle-filter-fields-title">
              <div className="parent-filter-popup__section-header">
                <h3 className="parent-filter-popup__section-title" id="bundle-filter-fields-title">
                  Filter Fields
                </h3>
            ...
            <section className="parent-filter-popup__section" aria-labelledby="bundle-filter-values-title">
              <div className="parent-filter-popup__section-header">
                <h3 className="parent-filter-popup__section-title" id="bundle-filter-values-title">
                  Filter Values
                </h3>
              </div>
              ...
              {selectedFilterKeys.length > 0 ? (
                <div className="parent-filter-popup__value-grid">
                  {hasSelectedSortFilter ? (
                    ...
                    >
                      <InputLabel id="bundle-dialog-sort-label" shrink>
                        Sort By
                      </InputLabel>
                      <Select
                        labelId="bundle-dialog-sort-label"
                        id="bundle-dialog-sort-select"
                        value={sortValue}
                        label="Sort By"
                  ...
                  {selectedFilterConfigs.map((filterConfig) => {
                    ...
                    return (
                      <FormControl
                        ...
                        className="parent-table-mui-filter parent-filter-popup__field"
                      >
                        <InputLabel id={`bundle-dialog-filter-${filterConfig.key}-label`} shrink>
                          {filterConfig.label}
                        </InputLabel>
                        <Select
                          labelId={`bundle-dialog-filter-${filterConfig.key}-label`}
                          id={`bundle-dialog-filter-${filterConfig.key}-select`}
                          value={selectedValue}
...
export default DialogFilterBundle
