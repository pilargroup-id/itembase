...
function getSelectedFilterSummary(filterFieldOptions, selectedFilterKeys) {
  ...
  const filterLabels = filterKeys.map((filterKey) => getParentFilterLabel(filterFieldOptions, filterKey))
...
function DialogFilterParent({
  isOpen = false,
  eyebrow = 'Filter',
  title = 'Parent Table Filter',
  filterConfigs = [],
  filterFieldOptions = [],
  selectedFilterKeys = [],
  ...
  const dialogNode = (
    <div className="dashboard-popup-overlay" role="presentation" onClick={onClose}>
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="dialog-filter-parent-title"
        onClick={(event) => event.stopPropagation()}
      >
        <div className="dashboard-popup__header">
          <div>
            <p className="dashboard-popup__eyebrow">{eyebrow}</p>
            <h2 className="dashboard-popup__title" id="dialog-filter-parent-title">
              {title}
            </h2>
          </div>
        ...
        <div className="dashboard-popup__body parent-filter-popup__body">
          <div className="parent-filter-popup__layout">
            <section className="parent-filter-popup__section" aria-labelledby="parent-filter-fields-title">
              <div className="parent-filter-popup__section-header">
                <h3 className="parent-filter-popup__section-title" id="parent-filter-fields-title">
                  Filter Fields
                </h3>
            ...
            <section className="parent-filter-popup__section" aria-labelledby="parent-filter-values-title">
              <div className="parent-filter-popup__section-header">
                <h3 className="parent-filter-popup__section-title" id="parent-filter-values-title">
                  Filter Values
                </h3>
              </div>
              ...
              {selectedFilterKeys.length > 0 ? (
                <div className="parent-filter-popup__value-grid">
                  {hasSelectedSortFilter ? (
                    ...
                    >
                      <InputLabel id="parent-dialog-sort-label" shrink>
                        Sort By
                      </InputLabel>
                      <Select
                        labelId="parent-dialog-sort-label"
                        id="parent-dialog-sort-select"
                        value={sortValue}
                        label="Sort By"
...
export default DialogFilterParent
