...
import { Download01, FileText01, Upload01, XClose } from '../../template/TemplateIcons.jsx'
...
function DialogImportMaster({
  ...
  const eyebrow = `Import ${masterLabel}`
  const title = `Preview Import ${masterLabel}`
  ...
  const handleClose = async () => {
    ...
    if (previewToken && !commitResponse) {
      ...
      try {
        await api.itemData.imports.masterCancelPreview(previewToken)
      } catch (error) {
        console.error(error)
      } finally {
  ...
  const handleFileSelect = async (file) => {
    ...
    try {
      const response = await api.itemData.imports.masterPreview(type, formData)
  ...
  const handleDownloadTemplate = async () => {
    ...
    try {
      const blob = await api.itemData.templates.masters(type)
  ...
  const handleCommit = async () => {
    ...
    try {
      const response = await api.itemData.imports.masterCommit({
        preview_token: previewToken,
      })
      const data = getResponseData(response)

      setCommitResponse(response)
      onImported?.(data, response)

      if (data?.error_file_token) {
        const errorFile = await api.itemData.imports.masterErrors(data.error_file_token)

        saveBlob(errorFile, `${type}-import-errors.xlsx`)
      } else {
        onClose?.()
      }
    } catch (error) {
  ...
  const dialogNode = (
    <div className="dashboard-popup-overlay" role="presentation" onClick={handleClose}>
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="dialog-import-master-title"
        onClick={(event) => event.stopPropagation()}
      >
        <div className="dashboard-popup__header">
          <div>
            <p className="dashboard-popup__eyebrow">{eyebrow}</p>
            <h2 className="dashboard-popup__title" id="dialog-import-master-title">
              {title}
            </h2>
          </div>
        ...
        <div className="dashboard-popup__body parent-import-popup__body">
          <div className="parent-import-template-actions">
            ...
            <button
              type="button"
              className="users-table-card__action parent-table-tool-button parent-table-tool-button--download"
              onClick={handleDownloadTemplate}
              disabled={isDownloadingTemplate}
              aria-busy={isDownloadingTemplate}
              aria-label={`Download template ${masterLabel}`}
            >
              <Download01 size={18} aria-hidden="true" />
...
export default DialogImportMaster
