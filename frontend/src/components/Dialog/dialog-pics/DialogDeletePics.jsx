...
function DialogDeletePics({
  ...
  const displayName = useMemo(() => getPicsDisplayName(pics), [pics])
  ...
  const handleDelete = async () => {
    ...
    try {
      await api.pics.remove(deleteId)
      onDeleted?.(pics)
      onConfirm?.(pics)
      notifySuccess('Pics deleted successfully.')
    } catch (error) {
  ...
  const dialogNode = (
    ...
    >
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="dialog-delete-pics-title"
        onClick={(event) => event.stopPropagation()}
      >
        <div className="dashboard-popup__header">
          <div>
            <p className="dashboard-popup__eyebrow">{eyebrow}</p>
            <h2 className="dashboard-popup__title" id="dialog-delete-pics-title">
              {title}
            </h2>
          </div>
...
export default DialogDeletePics
