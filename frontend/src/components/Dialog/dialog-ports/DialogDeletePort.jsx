...
function DialogDeletePort({
  ...
  const handleDelete = async () => {
    const deleteId = getDeleteId(Port)
    ...
    try {
      ...
      setErrorMessage(error?.message || 'Gagal menghapus Port.')
    } finally {
      setIsSubmitting(false)
    }
  }
  ...
  const dialogNode = (
    ...
    >
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="dialog-delete-Port-title"
        onClick={(event) => event.stopPropagation()}
      >
        <div className="dashboard-popup__header">
          <div>
            <p className="dashboard-popup__eyebrow">{eyebrow}</p>
            <h2 className="dashboard-popup__title" id="dialog-delete-Port-title">
              {title}
            </h2>
          </div>
        ...
        <div className="dashboard-popup__body">
          ...
          <p className="dashboard-popup__text">
            Tindakan ini akan menghapus data Port dari master Ports.
          </p>
          {errorMessage ? (
...
export default DialogDeletePort
