...
function DialogCreatePicUser({
  isOpen = false,
  eyebrow = 'Create Pic User',
  title = 'Create Pic User',
  onClose,
  onCreated,
}) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const createdPicUser = await api.picUsers.create(payload)

      onCreated?.(createdPicUser)
      notifySuccess('PIC user created successfully.')
      handleClose()
    } catch (error) {
  ...
  const dialogNode = (
    ...
    >
      <form
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="dialog-create-pic-user-title"
      ...
      >
        <div className="dashboard-popup__header">
          <div>
            ...
            <h2 className="dashboard-popup__title" id="dialog-create-pic-user-title">
              {title}
            </h2>
          </div>
...
export default DialogCreatePicUser
