...
function ButtonDownloadParent({
  ...
  const handleClick = async (event) => {
    ...
    try {
      const blob = await api.itemData.templates.parents()

      saveBlob(blob, 'item-parent-template.xlsx')
    } catch (error) {
      console.error(error)
    } finally {
...
export default ButtonDownloadParent
